...
export function jsonReader({ flow }: any): BlockList[] {
  ...
  keys.forEach((key) => {
    ...
    let dynamicContent = flow[key].$enteringCustomActions
      .find(({ $title }: any) => $title.includes('dynamicContent'))
    if (dynamicContent !== undefined) {
      try {
        // eslint-disable-next-line @typescript-eslint/no-implied-eval, no-new-func
        const run = new Function('return ' + dynamicContent.settings.source + '()')
        const json = run()
        if (json?.interactive?.type === 'list' || json?.interactive?.type === 'button') {
          dynamicContent = {
            type: json.interactive.type,
            body: json.interactive.body,
            buttons: json.interactive.action
          }
        }
